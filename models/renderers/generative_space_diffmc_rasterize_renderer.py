...
@threestudio.register("generative-space-diffmc-rasterize-renderer")
class GenerativeSpaceDiffMCRasterizeRenderer(NVDiffRasterizer):
    @dataclass
    ...
    def isosurface(self, space_cache: Float[Tensor, "B ..."]) -> List[Mesh]:
        ...
        for index in range(sdf_batch.shape[0]):
            ...
            if index > 0 and self.cfg.isosurface_method == "diffmc":
                ...
                mesh = getattr(self, f"isosurface_helper_{index}")(sdf, deformation, isovalue = iso_value)
            else:
                mesh = self.isosurface_helper(sdf, deformation, isovalue = iso_value)
